...
void MovePengNE(int ID, int Spaces, int NumOfRows, int NumOfCols, int FishArray[NumOfRows][NumOfCols], int PengRows, int PengArray[PengRows][3]) {
    ...
    idRow = WhichPenguin(ID, PengRows, PengArray); //Search for the right row in the array so we don't have to check it every incrementation of Spaces
    ...
    for(i=0; i < Spaces; i++) {
        ...
        if(PengArray[idRow][1] % 2) {
            ...
            if(CheckMove(--Row, ++Col, NumOfCols, FishArray, PengRows, PengArray) == 1) {
